...
// Lazy init to avoid deployment timeout during module analysis
...
async function findMedicationByName(name: string): Promise<any> {
  const nameLower = name.toLowerCase().trim();

  const snapshot = await getDb().collection('medications').get();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function getMedicationInteractions(medicationName: string) {
  ...
  const [asDrugA, asDrugB] = await Promise.all([
    getDb().collection('interactions').where('drugA', '==', medDoc.ref).get(),
    getDb().collection('interactions').where('drugB', '==', medDoc.ref).get()
  ]);
