...
export async function searchMedications(searchTerm: string, maxResults: number = 20) {
  ...
  // Get all medications
  const snapshot = await getDb().collection('medications')
    .limit(100)
    .get();
...
export async function getMedicationById(medicationId: string) {
  try {
    const doc = await getDb().collection('medications').doc(medicationId).get();
...
export async function getMedicationsByIds(medicationIds: string[]) {
  ...
  try {
    ...
    const chunks: any[] = [];
    for (let i = 0; i < medicationIds.length; i += 10) {
      const chunk = medicationIds.slice(i, i + 10);
      const docs = await Promise.all(
        chunk.map(id => getDb().collection('medications').doc(id).get())
      );
      chunks.push(...docs);
    }
